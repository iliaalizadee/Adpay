...
public class WelcomeActivity extends AppCompatActivity {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Method.forceRTLIfSupported(getWindow(), WelcomeActivity.this);
        ...
        if (!prefManager.isFirstTimeLaunch()) {
            if(DateHandler.diffdays(prefManager.getLastDate()) == 1){
                prefManager.setFreqDays(prefManager.getFreqDays()+1);
            }else if(DateHandler.diffdays(prefManager.getLastDate()) > 1){
                prefManager.setFreqDays(0);
            }
            if(prefManager.getLastDate() != DateHandler.gettime()){
                prefManager.setLastDate(DateHandler.gettime());
                prefManager.setAllZero();
            }
            launchHomeScreen();
            finish();
        }
    ...
    private void addBottomDots(int currentPage) {
        ...
        for (int i = 0; i < dots.length; i++) {
            ...
            dots[i].setTextColor(getResources().getColor(R.color.array_dot_inactive));
